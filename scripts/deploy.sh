...
EB_BUCKET=[elasticbeanstalk-us-east-2-997840975440]
# Set the default region for aws cli
aws configure set default.region us-east-2
# Log in to ECR
eval $(aws ecr get-login --no-include-email --region us-east-2)
# Build docker image based on our production Dockerfile
docker build -t facompany/mm-prod .
# tag the image with the Travis-CI SHA
docker tag facompany/mm-prod:latest 997840975440.dkr.ecr.us-east-2.amazonaws.com/mm
:$TRAVIS_COMMIT
# Push built image to ECS
docker push 997840975440.dkr.ecr.us-east-2.amazonaws.com/mm
:$TRAVIS_COMMIT
...
aws elasticbeanstalk create-application-version --application-name MegamarketsEnv-prod1 --version-label $TRAVIS_COMMIT --source-bundle S3Bucket=$EB_BUCKET,S3Key=mm-prod-deploy.zip
